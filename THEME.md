...
S1 已完成:`ZH` / `EN` 两表全部换成山海名词与带原文的横幅;`LORE` 表每条带出处与原文,道藏"妖录"改为卡片显示出处 + 原文;标题副题"云隐灵谷,山海之间"。

S2 / S3 已完成:`shanhai_creatures.py` + `shanhai_scenery.py` 拼进 `build_kit_shanhai.py`(73 个 Kit,2.08 MB);地物与出处:琅玕树(槐江之山"多琅玕")、沙棠(昆仑之丘)、柘木(发鸠之山"多柘木")、桢木(太山)、桂 / 迷榖 / 祝余(招摇之山)、武罗石像(青要之山)、帝台之棋(高前之山)、昆仑门("面有九门,门有开明兽守之")、弱水、炎火之山、精卫石堆、玄狐(幽都)、扶桑十日、汤谷、丹穴、青丘。迷榖"其华四照"做成夜里发光的花(`Lantern_MgFlower`);扶桑十日用 `Fire_FsSun`。

S4 已完成:"见则"征兆系统(见 `DESIGN.md` 征兆表),妖潮横幅按兽种引原文,妖录扩到 14 条(加当康 / 凤皇 / 烛龙 / 精卫),拾取物配色 琅玕青玉 / 丹粟赤金 / 视肉肉红。
